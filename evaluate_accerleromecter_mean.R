...
png(file="ENMO_Score_mean_sex.png")
...
rheuma_data %>%
  ...
  #mutate(sex = ifelse(female == 1, 'f', 'm')) 
    ...
    geom_point() +
    ggtitle('Differences in activity with and without rheuma') +
    ylab('mean of ENMO (gravity)') +
    geom_smooth(method = 'lm', se=TRUE) #se = TRUE -> with 95 conf. interval

png(file="Rheuma_ENMO_Mean_age.png")
...
png(file="Rheuma_ENMO_Mean_maxgrip.png")
