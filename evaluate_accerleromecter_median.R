...
ggplot(sum_sex, aes(x=female, y=median , fill=factor(female))) +
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=median-ci, ymax=median+ci),
                size =.3,
                width =.2, 
                position=position_dodge(.9)) +
  xlab("sex") +
  ylab("ENMO Score median") + 
  scale_fill_hue(name=" ", labels=c("male","female"))

png(file="ENMO_Score_median_sex.png")
...
grip_rheuma %>%
  mutate(rheuma = ifelse(ph006d19 == 1 | ph006d20 == 1, 'yes', 'no')) %>%
  ggplot(aes(x=maxgrip,y=median, color=rheuma)) + 
  geom_point() +
  ggtitle('Differences in activity with and without rheuma depending on age') +
  ylab('mean of ENMO (gravity)') +
  geom_smooth(method = 'lm', se=TRUE) #se = TRUE -> with 95 conf. interval
png(file="ENMO_MEDIAN_MAXGRIP.png")
